...
import {
  AiProposalDoc,
  AiProposalJsonSchema,
  buildDynamicProposalSchema,
} from '../schemas/aiProposal.schema';
...
export class AIProposalGenerator {
  ...
  async generateProposal(
    ...
    const { meeting, selectedServices, pmNote, additionalInstructions, model } =
      options;

    let lastError: Error | null = null;

    // Get proposal summary early to determine ROI data presence
    const proposalSummary = this.calculateProposalSummary(
      meeting,
      selectedServices
    );
    ...
    for (let attempt = 0; attempt < this.retryCount; attempt++) {
      try {
        ...
        const response = await this.callOpenAIResponsesAPI({
          model:
            model || import.meta.env.VITE_AI_MODEL || 'gpt-5-mini-2025-08-07',
          messages,
          seed: 7,
        ...
        if (!sections) {
          throw new Error(
            'Invalid response format from AI - check console for detailed validation errors'
          );
        }
        ...
        console.warn(
          `[AI Proposal] Generation attempt ${attempt + 1} failed:`,
          lastError.message
        );
        ...
        if (
          ...
          console.error(
            '[AI Proposal] Rate limit or quota exceeded, stopping retries'
          );
          break;
        }
        ...
        if (lastError.message.includes('Invalid response format')) {
          console.log(
            `[AI Proposal] Validation failed on attempt ${attempt + 1}, will retry...`
          );
        }
    ...
    // All retries failed
    console.error(
      'AI proposal generation failed after all retries:',
      lastError
    );
    return {
      success: false,
      error:
        lastError?.message ||
        'Failed to generate proposal after multiple attempts',
    };
  }
   ...
   */
  private calculateProposalSummary(
    meeting: Meeting,
    selectedServices: SelectedService[]
  ): ProposalSummary {
    ...
    const potentialMonthlySavings =
      typeof meetingROI?.summary?.totalMonthlySaving === 'number' &&
      meetingROI.summary.totalMonthlySaving > 0
        ? meetingROI.summary.totalMonthlySaving
        : typeof meetingROI?.estimatedCostSavings === 'number' &&
            meetingROI.estimatedCostSavings > 0
          ? meetingROI.estimatedCostSavings
          : 0;

    return {
      totalServices: selectedServices.length,
      totalAutomations: selectedServices.filter(
        (s) => s.category === 'automations'
      ).length,
      totalAIAgents: selectedServices.filter((s) => s.category === 'ai_agents')
        .length,
      totalIntegrations: selectedServices.filter(
  ...
  private hasROIData(
    ...
    const meetingROI = meeting.modules?.roi;
    const totalMonthlySavingFromMeeting =
      meetingROI?.summary?.totalMonthlySaving;
    const estimatedCostSavingsFromMeeting = meetingROI?.estimatedCostSavings;
    const potentialMonthlySavingsFromSummary =
      proposalSummary?.potentialMonthlySavings; // This will now be populated by calculateProposalSummary

    // Consider ROI data present if any of the key fields indicate a positive saving
    return (
      (typeof totalMonthlySavingFromMeeting === 'number' &&
        totalMonthlySavingFromMeeting > 0) ||
      (typeof estimatedCostSavingsFromMeeting === 'number' &&
        estimatedCostSavingsFromMeeting > 0) ||
      (typeof potentialMonthlySavingsFromSummary === 'number' &&
        potentialMonthlySavingsFromSummary > 0)
    );
  }
    ...
    try {
      ...
      if (Array.isArray(parsed.services)) {
        parsed.services = parsed.services.map((service: any) => ({
          ...service,
          titleHe:
            typeof service.titleHe === 'string' ? service.titleHe.trim() : '',
          whyRelevantHe:
            typeof service.whyRelevantHe === 'string'
              ? service.whyRelevantHe.trim()
              : '',
          whatIncludedHe:
            typeof service.whatIncludedHe === 'string'
              ? service.whatIncludedHe.trim()
              : '',
        }));
      }
  ...
  private parseAndValidateResponse(data: string): AiProposalDoc | null {
    try {
      ...
      console.log(
        '[AI Proposal] Raw response received (first 500 chars):',
      ...
      if (
        ...
        !Array.isArray(sanitized.executiveSummary)
      ) {
        errors.push('Executive summary must be an array of strings');
      } else if (
        sanitized.executiveSummary.length < 2 ||
        sanitized.executiveSummary.length > 6
      ) {
        errors.push(
          `Executive summary must have 2-6 paragraphs (received ${sanitized.executiveSummary.length})`
        );
      } else {
        // Validate each paragraph with relaxed requirements
        for (let i = 0; i < sanitized.executiveSummary.length; i++) {
          if (
            typeof sanitized.executiveSummary[i] !== 'string' ||
            sanitized.executiveSummary[i].trim().length < 30
          ) {
            errors.push(
              `Executive summary paragraph ${i + 1} must have at least 30 characters (received ${sanitized.executiveSummary[i]?.length || 0})`
            );
          }
        }
      }
      ...
      if (!sanitized.services || !Array.isArray(sanitized.services)) {
        ...
        for (let i = 0; i < sanitized.services.length; i++) {
          ...
          if (
            !service.titleHe ||
            typeof service.titleHe !== 'string' ||
            service.titleHe.trim().length < 5
          ) {
            errors.push(
              `Service ${i + 1} must have a valid Hebrew title (received ${service.titleHe?.length || 0} chars)`
            );
          }
          if (
            !service.whyRelevantHe ||
            typeof service.whyRelevantHe !== 'string' ||
            service.whyRelevantHe.trim().length < 15
          ) {
            errors.push(
              `Service ${i + 1} whyRelevant must have at least 15 characters (received ${service.whyRelevantHe?.length || 0})`
            );
          }
          if (
            !service.whatIncludedHe ||
            typeof service.whatIncludedHe !== 'string' ||
            service.whatIncludedHe.trim().length < 15
          ) {
            errors.push(
              `Service ${i + 1} whatIncluded must have at least 15 characters (received ${service.whatIncludedHe?.length || 0})`
            );
          }
        }
      }
      ...
      if (
        !sanitized.financialSummary ||
        typeof sanitized.financialSummary !== 'object'
      ) {
        errors.push('Financial summary must be an object');
      } else {
        if (
          typeof sanitized.financialSummary.totalPrice !== 'number' ||
          sanitized.financialSummary.totalPrice <= 0
        ) {
          errors.push(
            `Financial summary must have valid total price > 0 (received ${sanitized.financialSummary.totalPrice})`
          );
        }
        if (
          typeof sanitized.financialSummary.totalDays !== 'number' ||
          sanitized.financialSummary.totalDays <= 0
        ) {
          errors.push(
            `Financial summary must have valid total days > 0 (received ${sanitized.financialSummary.totalDays})`
          );
        }
        if (
          sanitized.financialSummary.monthlySavings !== undefined &&
          (typeof sanitized.financialSummary.monthlySavings !== 'number' ||
            sanitized.financialSummary.monthlySavings < 0)
        ) {
          errors.push(
            `Monthly savings must be non-negative if provided (received ${sanitized.financialSummary.monthlySavings})`
          );
        }
        if (
          sanitized.financialSummary.expectedROIMonths !== undefined &&
          (typeof sanitized.financialSummary.expectedROIMonths !== 'number' ||
            sanitized.financialSummary.expectedROIMonths <= 0)
        ) {
          errors.push(
            `Expected ROI months must be positive if provided (received ${sanitized.financialSummary.expectedROIMonths})`
          );
        }
      }
      ...
      if (!sanitized.terms || !Array.isArray(sanitized.terms)) {
        ...
        for (let i = 0; i < sanitized.terms.length; i++) {
          if (
            typeof sanitized.terms[i] !== 'string' ||
            sanitized.terms[i].trim().length < 5
          ) {
            errors.push(
              `Term ${i + 1} must have at least 5 characters (received ${sanitized.terms[i]?.length || 0})`
            );
          }
        }
      }
      ...
      if (!sanitized.nextSteps || !Array.isArray(sanitized.nextSteps)) {
        ...
        for (let i = 0; i < sanitized.nextSteps.length; i++) {
          if (
            typeof sanitized.nextSteps[i] !== 'string' ||
            sanitized.nextSteps[i].trim().length < 5
          ) {
            errors.push(
              `Next step ${i + 1} must have at least 5 characters (received ${sanitized.nextSteps[i]?.length || 0})`
            );
          }
        }
      }
      ...
      if (errors.length > 0) {
        ...
        console.error(
          '[AI Proposal] Sanitized response structure:',
