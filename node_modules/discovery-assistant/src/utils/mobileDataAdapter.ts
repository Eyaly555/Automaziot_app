  ...
  return {
    ...
    systems: {
      ...
      integrations: {
        ...
        manualDataTransfer: mapTimeWastedToManualTransfer(
          automations.time_wasted
        ),
      },
      dataQuality: {
    ...
    roi: {
      ...
      timeSavings: {
        estimatedHoursSaved: String(
          mapTimeWastedToHours(automations.time_wasted) * 0.7
        ),
...
function mapDomainToSalesUseCases(
  domains: string[],
  channels: string[]
): string[] {
...
function mapDomainToServiceUseCases(
  domains: string[],
  channels: string[]
): string[] {
  const useCases: string[] = [];
...
function determineMainGoals(automations: any, crm: any): string[] {
  ...
  if (
    automations.time_wasted === 'over_4h' ||
    automations.time_wasted === '3-4h'
  ) {
  ...
  if (
    automations.biggest_pain === 'things_fall' ||
    automations.biggest_pain === 'no_tracking'
  ) {
...
function determineProcesses(automations: any): string[] {
  ...
  if (
    automations.processes.includes('lead_management') ||
...
function calculateIntegrationLevel(
  crm: any
