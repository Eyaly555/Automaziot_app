...
import { X, FileText, MessageSquare, CheckCircle, Clock, AlertCircle } from 'lucide-react';
 ...
 */
export const ConversationSummaryViewer: React.FC<ConversationSummaryViewerProps> = ({
  analysis,
  onClose
}) => {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b bg-gradient-to-r from-blue-50 to-blue-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-900">סיכום וניתוח שיחה</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <Card title="מידע כללי">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-medium text-gray-900">{analysis.audioFile.fileName}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium text-gray-900">
                  {analysis.transcription.language === 'he' ? 'עברית' : analysis.transcription.language}
                </p>
              </div>
              <div>
          ...
          {Object.keys(analysis.analysis.extractedFields).length > 0 && (
            <Card title="שדות שזוהו">
              <div className="space-y-4">
                {Object.entries(analysis.analysis.extractedFields).map(([moduleName, fields]) => {
                  ...
                  return (
                    <div key={moduleName} className="border-r-4 border-blue-200 pr-4">
                      <h4 className="font-semibold text-gray-900 mb-2 flex items-center gap-2">
                        <CheckCircle className="w-4 h-4 text-green-600" />
                        {moduleNames[moduleName] || moduleName}
                      </h4>
                      <div className="space-y-1 text-sm text-gray-600">
                        {Object.entries(fields as Record<string, any>).map(([fieldName, fieldValue]) => {
                          if (fieldValue === undefined || fieldValue === null || fieldValue === '') return null;
                          
                          let displayValue: string;
                          if (Array.isArray(fieldValue)) {
                            displayValue = fieldValue.length > 0 ? `${fieldValue.length} פריטים` : 'ריק';
                          ...
                          return (
                            <p key={fieldName} className="flex items-start gap-2">
          ...
          {/* Next Steps Card */}
          {analysis.analysis.nextSteps && analysis.analysis.nextSteps.length > 0 && (
          ...
          <Card title="תמלול מלא">
            ...
            <div className="mt-3 text-xs text-gray-500 flex items-center gap-2">
              ...
              <span>התמלול נוצר אוטומטית על ידי OpenAI Whisper ועשוי להכיל אי דיוקים</span>
            </div>
          </Card>
        </div>
