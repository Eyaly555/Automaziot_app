...
import { TextField, RadioGroup, CheckboxGroup, TextAreaField } from '../../Common/FormFields';
...
export const DetailedSystemCard: React.FC<DetailedSystemCardProps> = ({
  ...
  const systemDetails = getSystemDetails(system.category, system.specificSystem);
  ...
  const handleSaveIntegration = (integration: Omit<SystemIntegrationNeed, 'id'>) => {
  ...
  const handleRemoveIntegration = (integrationId: string) => {
    onUpdate({
      integrationNeeds: system.integrationNeeds.filter(int => int.id !== integrationId)
    });
  };
    ...
    >
      ...
      <div className="space-y-6 mt-2">
        ...
        <TextField
          ...
          onChange={(value) => onUpdate({ recordCount: parseInt(value) || undefined })}
        ...
        <TextField
          ...
          onChange={(value) => onUpdate({ monthlyUsers: parseInt(value) || undefined })}
        ...
        <div>
          ...
          <CheckboxGroup
            label=""
            options={commonPainPoints.map(point => ({ value: point, label: point }))}
        ...
        {criticalFeatures.length > 0 && (
          <div>
            ...
            <CheckboxGroup
              label=""
              options={criticalFeatures.map(feature => ({ value: feature, label: feature }))}
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded ${
                        ...
                        {integration.criticalityLevel === 'critical' ? 'קריטי' :
                         integration.criticalityLevel === 'important' ? 'חשוב' : 'רצוי'}
                      </span>
                    </div>
                    <div className="text-xs text-gray-600 space-y-0.5">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {integration.currentStatus === 'working' && (
                          <><CheckCircle className="w-3 h-3 text-green-600" /> עובד</>
                        )}
                        {integration.currentStatus === 'problematic' && (
                          <><AlertTriangle className="w-3 h-3 text-orange-600" /> בעייתי</>
                        )}
...
const IntegrationForm: React.FC<IntegrationFormProps> = ({ allSystems, onSave, onCancel }) => {
  const [targetSystemId, setTargetSystemId] = useState('');
  const [targetSystemName, setTargetSystemName] = useState('');
  const [integrationType, setIntegrationType] = useState<SystemIntegrationNeed['integrationType']>('api');
  const [frequency, setFrequency] = useState<SystemIntegrationNeed['frequency']>('daily');
  const [dataFlow, setDataFlow] = useState<SystemIntegrationNeed['dataFlow']>('bidirectional');
  const [criticalityLevel, setCriticalityLevel] = useState<SystemIntegrationNeed['criticalityLevel']>('important');
  const [currentStatus, setCurrentStatus] = useState<SystemIntegrationNeed['currentStatus']>('missing');
  ...
  const handleSubmit = () => {
    ...
    onSave({
      targetSystemId,
      targetSystemName: targetSystemName || allSystems.find(s => s.id === targetSystemId)?.specificSystem || '',
      integrationType,
      frequency,
      dataFlow,
      criticalityLevel,
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6" dir="rtl">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const system = allSystems.find(s => s.id === e.target.value);
