...
export const BlockerManagement: React.FC = () => {
  ...
  const handleResolveBlocker = (taskId: string, blockerId: string) => {
    if (confirm('Mark this blocker as resolved?')) {
      ...
      const taskBlockers = activeBlockers.filter(
        (b) => b.taskId === taskId && b.id !== blockerId
      );
        ...
        {activeBlockers.length > 0 ? (
          <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
            ...
            <div className="space-y-6">
              {Object.entries(blockersByTask).map(
                ([taskId, { task, blockers: taskBlockers }]) => (
                  <div
                    key={taskId}
                    ...
                    <div className="bg-red-50 px-6 py-4 border-b border-red-200">
                      <div className="flex items-start justify-between">
                        ...
                        <span className="px-3 py-1 bg-red-100 text-red-800 rounded-full text-sm font-medium">
                          {taskBlockers.length}{' '}
                          {taskBlockers.length === 1
                            ? t.activeSection.blocker
                            : t.activeSection.blockers}
                        </span>
                      </div>
                    </div>
                        ...
                        return (
                          <div key={blocker.id} className="px-6 py-4">
                            <div className="flex items-start justify-between mb-3">
                              <div className="flex-1">
                                <div className="flex items-center space-x-3 mb-2">
                                  ...
                                  <span className="text-sm text-gray-600 flex items-center">
                                    ...
                                    {daysAgo === 0
                                      ? t.today
                                      : `${t.daysAgo} ${daysAgo} ${daysAgo === 1 ? t.resolvedSection.day : t.resolvedSection.days}`}
                                  </span>
                                  {blocker.reportedBy && (
                                    <span className="text-sm text-gray-600 flex items-center">
                                      <User className="w-3 h-3 mr-1" />
                              ...
                              <Button
                                onClick={() =>
                                  handleResolveBlocker(taskId, blocker.id)
                                }
                                variant="success"
                                size="sm"
                              >
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-4 text-xs text-gray-500">
                          <span>
                            {t.resolvedSection.resolved}{' '}
                            {resolvedDate?.toLocaleDateString(
                              language === 'he' ? 'he-IL' : 'en-US'
                            )}
                          </span>
                          <span>
                            {t.resolvedSection.timeToResolve}: {daysToResolve}{' '}
                            {daysToResolve === 1
                              ? t.resolvedSection.day
                              : t.resolvedSection.days}
                          </span>
                          {blocker.reportedBy && (
                            <span>
                              {t.resolvedSection.reportedBy}{' '}
                              {blocker.reportedBy}
                            </span>
                          )}
                        </div>
