...
export const ProgressTracking: React.FC = () => {
  ...
  const overallMetrics = useMemo(() => {
    ...
    const variancePercent =
      completedHours > 0 ? Math.round((variance / completedHours) * 100) : 0;
  ...
  const teamMetrics = useMemo(() => {
    ...
    return assignees.map((assignee) => {
      ...
      const inProgress = assigneeTasks.filter(
        (t) => t.status === 'in_progress'
      ).length;
      const blocked = assigneeTasks.filter(
        (t) => t.status === 'blocked'
      ).length;

      const totalHours = assigneeTasks.reduce(
      ...
      return {
        ...
        progress:
          assigneeTasks.length > 0
            ? Math.round((completed / assigneeTasks.length) * 100)
            : 0,
      };
    });
  }, [tasks]);
  ...
  const typeBreakdown = useMemo(() => {
    ...
    return types
      .map((type) => ({
        ...
        completed: tasks.filter((t) => t.type === type && t.status === 'done')
          .length,
      }))
      .filter((t) => t.count > 0);
  }, [tasks]);
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">
            No development tracking data available
          </p>
        </div>
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-1">
            Manager dashboard for project insights
          </p>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          ...
          <Card>
            ...
            <div
              className={`text-3xl font-bold ${overallMetrics.variance > 0 ? 'text-red-600' : 'text-green-600'}`}
            >
              {overallMetrics.variance > 0 ? '+' : ''}
              {overallMetrics.variance}h
            </div>
            <div className="text-sm text-gray-500 mt-1">
              {overallMetrics.variancePercent > 0 ? '+' : ''}
              {overallMetrics.variancePercent}%
            </div>
          </Card>
          ...
          <div className="grid grid-cols-5 gap-4">
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-gray-500">
                {overallMetrics.totalTasks > 0
                  ? Math.round(
                      (overallMetrics.todoTasks / overallMetrics.totalTasks) *
                        100
                    )
                  : 0}
                %
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-1">
                          ...
                          <span
                            className={`px-2 py-1 rounded text-xs ${statusColors[sprint.status]}`}
                          >
                            {sprint.status}
                          </span>
                        </div>
                    ...
                    <div className="flex items-center justify-between mt-2 text-xs text-gray-500">
                      ...
                      <span>
                        {sprint.completedHours}/{sprint.totalHours}h
                      </span>
                    </div>
                  </div>
                );
                ...
                <div key={type}>
                  ...
                  <div className="bg-gray-100 rounded-full h-2 overflow-hidden">
                    <div
                      className="bg-blue-500 h-full transition-all duration-500"
                      style={{
                        width: `${count > 0 ? (completed / count) * 100 : 0}%`,
                      }}
                    />
                  </div>
                </div>
