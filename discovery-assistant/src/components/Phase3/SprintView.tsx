...
export const SprintView: React.FC = () => {
  ...
  const sprintStats = useMemo(() => {
    ...
    const completedHours = sprintTasks.filter(t => t.status === 'done').reduce((sum, t) => sum + t.estimatedHours, 0);

    return { total, completed, inProgress, blocked, totalHours, completedHours };
  }, [sprintTasks]);
  ...
  const burndownData = useMemo(() => {
    ...
    for (let day = 0; day <= Math.min(daysPassed, totalDays); day++) {
      ...
      const actualRemaining = sprintStats.totalHours - sprintStats.completedHours;
                      ...
                      <>
                        ...
                        <polyline
                          points={burndownData
                            .map((d, i) => {
                              ...
                              const y = 100 - (d.ideal / sprintStats.totalHours) * 100;
                              return `${x}%,${y}%`;
                            })
                            .join(' ')}
                          fill="none"
                        ...
                        <polyline
                          points={burndownData
                            .map((d, i) => {
                              const x = (i / (burndownData.length - 1)) * 100;
                              const y = 100 - (d.actual / sprintStats.totalHours) * 100;
                              return `${x}%,${y}%`;
                            })
                            .join(' ')}
                          fill="none"
