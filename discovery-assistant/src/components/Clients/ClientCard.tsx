...
export const ClientCard: React.FC<ClientCardProps> = ({
  ...
  const getPriorityLevel = (client: ZohoClientListItem) => {
    if (client.overallProgress < 30)
      return { level: 'high', color: 'text-red-600', bgColor: 'bg-red-50' };
    if (client.overallProgress > 80)
  ...
  return (
    ...
    >
      {/* Priority Indicator */}
      <div
        className={`absolute top-0 right-0 w-2 h-full ${priority.bgColor}`}
      />
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          {/* Priority Badge */}
          <div
            className={`px-2 py-1 rounded-full text-xs font-semibold ${priority.bgColor} ${priority.color}`}
          >
            {priority.level === 'high'
              ? 'דחוף'
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        {client.discoveryModulesCompleted !== undefined &&
          client.discoveryModulesCompleted > 0 && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 text-center">
              <div className="text-lg font-bold text-blue-700">
                {client.discoveryModulesCompleted}
              </div>
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-center">
          <div className="text-lg font-bold text-gray-700">
            {Math.floor(
              (new Date().getTime() - new Date(client.created).getTime()) /
                (1000 * 60 * 60 * 24)
            )}
          </div>
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg px-3 py-2 text-center">
          <div className="text-lg font-bold text-green-700">
            {client.lastModified
              ? Math.floor(
                  (new Date().getTime() -
                    new Date(client.lastModified).getTime()) /
                    (1000 * 60 * 60 * 24)
                )
              : '?'}
          </div>
