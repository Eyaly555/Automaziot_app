    ...
    <div className="space-y-6" dir="rtl">
      <Card title="שירות #7: תיאום פגישות אוטומטי">
        <div className="space-y-6">
          ...
          {(calendarSystem.isAutoPopulated || crmSystem.isAutoPopulated || alertEmail.isAutoPopulated) && (
          ...
          {/* Conflict Warnings */}
          {(calendarSystem.hasConflict || crmSystem.hasConflict || alertEmail.hasConflict) && (
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">סוג פגישה</label>
                <select value={config.meetingType} onChange={(e) => setConfig({ ...config, meetingType: e.target.value as any })}
              ...
              <div>
                ...
                <select value={config.duration} onChange={(e) => setConfig({ ...config, duration: parseInt(e.target.value) })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">תזכורות</label>
              <div className="space-y-2">
                <label className="flex items-center">
                  <input type="checkbox" checked={config.reminderEmail}
                    onChange={(e) => setConfig({ ...config, reminderEmail: e.target.checked })} className="mr-2" />
                  <span className="text-sm">תזכורת במייל</span>
                </label>
                <label className="flex items-center">
                  <input type="checkbox" checked={config.reminderSMS}
                    onChange={(e) => setConfig({ ...config, reminderSMS: e.target.checked })} className="mr-2" />
                  <span className="text-sm">תזכורת ב-SMS</span>
                </label>
                <label className="flex items-center">
                  <input type="checkbox" checked={config.autoReschedule}
                    onChange={(e) => setConfig({ ...config, autoReschedule: e.target.checked })} className="mr-2" />
                  <span className="text-sm">תיאום אוטומטי מחדש במקרה של ביטול</span>
                </label>
              </div>
            </div>
