...
export function AIPredictiveSpec() {
  ...
  const { saveData, isSaving, saveError } = useAutoSave({
    moduleId: 'ai-predictive',
    immediateFields: ['aiModel', 'dataSource', 'predictionType'], // Critical configuration fields
    debounceMs: 1000,
    onError: (error) => {
      console.error('Auto-save error in AIPredictiveSpec:', error);
    }
  });
