...
export function ImplProjectManagementSpec() {
  ...
  const { saveData, isSaving, saveError } = useAutoSave({
    moduleId: 'impl-project-management',
    immediateFields: ['platform'], // Critical identifier
    debounceMs: 1000,
    onError: (error) => {
      console.error('Auto-save error in ImplProjectManagementSpec:', error);
    }
  });
    ...
    await saveData({
      implementationSpec: {
        ...
        systemImplementations: [
          ...(currentMeeting?.implementationSpec?.systemImplementations || []).filter((s: any) => s.serviceId !== 'impl-project-management'),
          {
            serviceId: 'impl-project-management',
            serviceName: 'הטמעת ניהול פרויקטים',
            requirements: config,
