...
export function ImplWorkflowPlatformSpec() {
  ...
  const { saveData, isSaving, saveError } = useAutoSave({
    moduleId: 'impl-workflow-platform',
    immediateFields: ['platform'], // Critical identifier
    debounceMs: 1000,
    onError: (error) => {
      console.error('Auto-save error in ImplWorkflowPlatformSpec:', error);
    }
  });

  useEffect(() => {
    const category = currentMeeting?.implementationSpec?.systemImplementations || [];
    const existing = category.find((s: any) => s.serviceId === 'impl-workflow-platform');
  ...
  useEffect(() => {
    if (config.platform) { // Only save if we have basic data
      const category = currentMeeting?.implementationSpec?.systemImplementations || [];
      const updated = category.filter((s: any) => s.serviceId !== 'impl-workflow-platform');
    ...
    await saveData({
      implementationSpec: {
        ...
        systemImplementations: [
          ...(currentMeeting?.implementationSpec?.systemImplementations || []).filter((s: any) => s.serviceId !== 'impl-workflow-platform'),
          {
            serviceId: 'impl-workflow-platform',
            serviceName: 'הטמעת פלטפורמת Workflow',
            requirements: config,
