...
export const generateTechnicalSpec = (
  meeting: Meeting
): TechnicalSpecification => {
  const systems = meeting.modules.systems?.detailedSystems || [];
  const allIntegrations = systems.flatMap((sys) =>
    sys.integrationNeeds.map((int) => ({
      ...int,
      sourceSystem: sys.specificSystem,
    }))
  );

  const criticalPainPoints = systems.reduce(
    (acc, sys) => acc + sys.mainPainPoints.length,
    0
  );
    ...
    migrationPriority:
      sys.migrationWillingness === 'eager'
        ...
        : sys.migrationWillingness === 'open'
          ? 'medium'
          : sys.migrationWillingness === 'reluctant'
            ? 'low'
            : 'none',
  }));
};

const generateIntegrationMap = (
  systems: DetailedSystemInfo[]
): IntegrationMapItem[] => {
...
const generateAutomationOpportunities = (
  ...
  if (missingIntegrations.length > 0) {
    opportunities.push({
      ...
      affectedSystems: [
        ...new Set(missingIntegrations.map((int) => int.targetSystemName)),
      ],
  ...
  if (
    meeting.modules.systems?.integrations?.manualDataTransfer &&
  ...
  const lowSatisfactionSystems = systems.filter(
    (s) => s.satisfactionScore <= 2
  );
...
const generateN8NWorkflows = (
  ...
  if (crmSystem) {
    workflows.push({
      ...
      nodes: [
        ...
        {
          type: crmSystem.specificSystem,
          action: 'Get lead details',
          system: crmSystem.specificSystem,
        },
  ...
  const criticalIntegrations = systems.flatMap((s) =>
    s.integrationNeeds.filter(
      (int) =>
        int.criticalityLevel === 'critical' && int.currentStatus === 'missing'
    )
  );
  ...
  if (criticalIntegrations.length > 0) {
    ...
    if (sourceSystem) {
      workflows.push({
        ...
        trigger:
          integration.frequency === 'realtime'
            ? 'Webhook'
            : `Schedule - ${integration.frequency}`,
        nodes: [
          {
            type: 'Trigger',
            action:
              integration.frequency === 'realtime' ? 'Webhook' : 'Schedule',
          },
          {
            type: sourceSystem.specificSystem,
            action: 'Get data',
            system: sourceSystem.specificSystem,
          },
          { type: 'Function', action: 'Transform data' },
          {
            type: integration.targetSystemName,
            action: 'Update records',
            system: integration.targetSystemName,
          },
...
const generateTechnicalRequirements = (
  systems: DetailedSystemInfo[]
...
const getAuthRequirements = (sys: DetailedSystemInfo): string[] => {
  ...
  return reqs.length > 0
    ? reqs
    : ['יש לבדוק דרישות Authentication במסמכי המערכת'];
};
...
const getRateLimit = (sys: DetailedSystemInfo): string => {
  ...
  if (sys.specificSystem.includes('Salesforce'))
    return '15,000 API calls/day (Enterprise)';
  if (sys.specificSystem.includes('HubSpot')) return '100 requests/10 seconds';
  if (sys.specificSystem.includes('Zoho'))
    return '5,000 API calls/day (Standard)';
...
const getDocumentationLink = (sys: DetailedSystemInfo): string => {
  ...
  return (
    links[sys.specificSystem.toLowerCase().replace(' ', '_')] ||
...
const generateImplementationPlan = (
  ...
  const criticalIntegrations = integrations.filter(
    (int) => int.criticalityLevel === 'critical'
  );
  if (criticalIntegrations.length > 0) {
    phases.push({
      phase: 2,
      ...
      duration: '2-3 שבועות',
      tasks: criticalIntegrations
        .slice(0, 3)
        .map((int) => `בניית workflow: ${int.from} → ${int.to}`)
        .concat([
          'בדיקות end-to-end',
  ...
  phases.push({
    ...
    dependencies: [`Phase ${phases.length}`],
    deliverables: ['מערכת מאופטמת בפרודקשן', 'הדרכות למשתמשים', 'מסמכי תחזוקה'],
  });
