...
6. צור Refresh Token באמצעות Authorization Code Flow`,
    notes:
      'שים לב: Data Center יכול להיות .com, .eu, .in, .com.au - חשוב לבדוק איזה Data Center החשבון נמצא בו',
  },
  ...
  salesforce: {
    ...
    rateLimits:
      '15,000 API calls per 24 hours (Enterprise), 5,000 (Professional)',
    documentation:
...
6. המתן 2-10 דקות לאקטיבציה`,
    notes:
      'Instance URL משתנה לפי הארגון (e.g., na50, eu11, ap5). בדוק ב-URL של החשבון שלך',
  },
...
3. שים לב ל-Company Domain (ה-subdomain שלך)`,
  ...
  monday: {
    ...
    rateLimits:
      '60 requests per minute (Standard), 120 rpm (Pro), 240 rpm (Enterprise)',
...
5. העתק Bot User OAuth Token`,
  ...
  'google-workspace': {
    ...
    rateLimits:
      'Varies by service (typically 100-1000 requests per 100 seconds)',
 ...
 */
export const getSystemAuthTemplate = (
  systemName: string
): SystemAuthTemplate | null => {
  ...
  // Try partial match
  const partialMatch = Object.keys(SYSTEM_AUTH_TEMPLATES).find(
